...
class BaseApi:
	...
	def generate_image(self, text: str) -> str:
		return self.client.images.generate(model="gemini", prompt=text)
	...
	def perform_task(self, text: str, role: str = "user", depth: int = 0):
		...
		if depth > self.max_depth:
			...
			raise TimeoutError(
				f"DEPTH_EXCEEDED, Maxiumum specified depth of {self.max_depth} exceeded. The conversation has been saved to ./{error_filename}"
			)
	...
	def determine_command(self, content: str) -> str:
		try:
			...
			print(e)
			return f"INVALID_COMMAND_FORMAT ({e}) - THE ONLY VALID COMMANDS ARE {self.available_commands} IN THE FORMAT MENTIONED IN YOUR CONTEXT. PLEASE FOLLOW THE FORMAT STRICTLY AND RESPOND WITH IT IN THE NEXT MESSAGE."
